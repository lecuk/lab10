...
{
	return (strstr(((Student*)item)->firstName, s_findByFirstName_substring) != NULL);
}

Group* query_findByFirstName(const Group* group, const char* name)
{
	s_findByFirstName_substring = name;
	return group_fromAnotherGroup("Result", s_findByFirstName_condition, group);
}
...
{
	return (strstr(((Student*)item)->lastName, s_findByLastName_substring) != NULL);
}

Group* query_findByLastName(const Group* group, const char* name)
{
	s_findByLastName_substring = name;
	return group_fromAnotherGroup("Result", s_findByLastName_condition, group);
}
...
Group* query_findByGrades(const Group* group, const float* grades, const size_t size)
{
	...
	return group_fromAnotherGroup("Result", s_findByGrades_condition, group);
}
...
Group* query_findBySum(const Group* group, const float min, const float max)
{
	s_findBySum_min = min;
	s_findBySum_max = max;
	return group_fromAnotherGroup("Result", s_findBySum_condition, group);
}
...
Group* query_findByMean(const Group* group, const float min, const float max)
{
	...
	return group_fromAnotherGroup("Result", s_findByMean_condition, group);
}
